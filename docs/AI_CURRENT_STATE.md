...
- Модификаторы: воин +0.1, завоеватель +0.2, осторожный +0.1, строитель −0.1…−0.2,
  директива вассалу «строить латников» +0.5; у админ/кочевых при малом долге
  `max = 3` (практически не распускают).
- ⚠️ **`0.40`/`0.60` — это БАЗА (граф/ранняя эпоха).** Король+ в High/Late medieval
  с достаточным доходом получает к `ideal` +0.2 (+0.2) → цель до **0.8**, и к `max`
  +0.2 (+0.2) → роспуск лишь выше **~0.9**. Крупные поздние державы держат куда
  большую постоянную армию латников. Полная разбивка модификаторов — в
  `docs/AI_ECONOMY.md` §1.3.
...
### Помесячный леджер (граф, 100 золота, +10/мес) — с реальными ценами

Точные цены латников из `00_men_at_arms_values.txt` (найм / мирное содержание):
тяжёлая пехота `90 / 0.4`, копейщики `75 / 0.3`, лучники `55 / 0.2`,
застрельщики `45 / 0.15`. Рекомендованное число полков
(`00_suggestion_values.txt`) = `ярус − 1` → у **графа = 1 полк**. Цена здания
принята `150` (точные — в `common/buildings/*`, не присылались).

Модель кошелька: цель военной казны = `max(50, 18×содержание)`; у графа без армии
содержание ≈ 0 → **казна-цель = 50** (вот почему не 108 — большой цифры там нет,
пока армия маленькая). Свободное золото делится `20% Развитие / 80% Война-сейчас`,
подушка Война-сейчас `200`, потолок `5000`. Допущение модели: найм латников
оплачивается из военного кошелька, постройки — из «Развития» (+ излишек военного
сверх подушки, когда войны нет).

| Мес | Казна | Войн.казна | Развитие | Война-сейчас | Действие |
|---|---|---|---|---|---|
| 0 | 100 | 50 | 10 | 40 | — нечем (нет 90 на полк, нет 150 на дом) |
| 1 | 110 | 50 | 12 | 48 | копит (+2 разв / +8 война) |
| 2–6 | …160 | 50 | …22 | …88 | копит |
| **7** | 170→**80** | 50 | 24 | 96→**6** | **найм 1 полк ТП −90**; содержание +0.4 → доход 9.6 |
| 8–32 | … | 50 | …~75 | …~200 | копит; военный кошелёк восстанавливает подушку 200 |
| **~41** | →~256 | 50 | ~0 | ~206 | **ПОСТРОЙКА #1 −150** (Развитие + излишек) |
| 42–64 | … | 50 | … | … | копит заново |
| **~65** | →~… | 50 | ~0 | ~… | **ПОСТРОЙКА #2 −150** |

Итог: 1 полк латников (как рекомендовано графу) куплен быстро (мес. 7, из военного
кошелька), а **2 здания — лишь к ~5–6 годам**, потому что в «Развитие» капает
только 20% × 10 ≈ 2/мес. Параллельно военный кошелёк **простаивает** (войны нет),
накапливая сотни золота впустую — это и есть «не сноуболится».

> Числа округлены; точные `%`, цены латников и пороги — из файлов, цена здания и
> разводка «найм из военного / стройка из развития» — разумное допущение модели
> (в файлах прямо не расписано), подлежит проверке в observer-игре.
